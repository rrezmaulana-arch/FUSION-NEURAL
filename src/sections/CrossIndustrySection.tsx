    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10">
            {MULTI_SECTOR_STEPS.map((step, i) => (
              ...
              >
                ...
                <h3 className="font-space font-bold text-fn-navy text-lg mb-2">{step.title}</h3>
                <p className="text-sm text-fn-navy/60 font-inter leading-relaxed">{step.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <motion.div
          ...
          className="mt-16 relative rounded-3xl p-10 overflow-hidden bg-slate-50"
          style={{
          ...
          <div className="flex flex-col md:flex-row flex-wrap justify-center items-center gap-12 relative z-10">
            {[
              ...
              <div key={stat.label} className="flex flex-col items-center gap-1">
                <p className="font-space font-bold text-4xl" style={{ color: stat.color }}>{stat.val}</p>
                <p className="text-xs text-fn-navy/50 font-inter text-center max-w-[120px]">{stat.label}</p>
              </div>
            ))}
          </div>
        </motion.div>
