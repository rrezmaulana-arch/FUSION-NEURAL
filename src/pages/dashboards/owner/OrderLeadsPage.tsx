...
export default function OrderLeadsPage() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'order_leads'), orderBy('createdAt', 'desc'));
    const unsub = onSnapshot(q, (snap) => {
      setLeads(snap.docs.map((d) => ({ id: d.id, ...d.data() } as OrderLead)));
    });
    return () => unsub();
  }, []);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-500 text-xs">
                      {lead.createdAt?.toDate().toLocaleString('id-ID') || 'Baru saja'}
                    </td>
                    <td className="px-6 py-4">
