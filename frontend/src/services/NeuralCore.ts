...
export class NeuralCore {
  ...
  /**
   * Initialize standard prompts in Firebase neural_configs collection if they don't exist
   */
  static async initCorePrompts() {
    ...
    try {
      for (const role of ['finance_brain', 'admin_brain', 'marketing_brain', 'manager_brain', 'chatbot', 'frontline_sales']) {
        const docRef = doc(db, 'neural_configs', role);
        const docSnap = await getDoc(docRef);
        if (!docSnap.exists()) {
          await setDoc(docRef, {
            role,
            prompt: DEFAULT_PROMPTS[role as keyof typeof DEFAULT_PROMPTS] ?? '',
          });
        }
      }
    } catch (error) {
  ...
  /**
   * Fetch a prompt from Firebase neural_configs for a specific agent.
   * Falls back to DEFAULT_PROMPTS if not found in DB.
   */
  ...
  /**
   * Applies a Neural Patch (updates agent's prompt) — menulis ke Firebase neural_configs.
   */
  static async applyNeuralPatch(agentId: string, newPrompt: string): Promise<void> {
    try {
      if (!db) throw new Error('Firebase not configured');
      await setDoc(doc(db, 'neural_configs', agentId), { role: agentId, prompt: newPrompt }, { merge: true });
      await FirebaseLogger.logAgentAction('Manager', 'NEURAL_PATCH', `Applied neural patch to ${agentId}`);
    } catch (error) {
      console.error(`Failed to apply Neural Patch to ${agentId}:`, error);
      throw error;
    }
  }
  ...
  static async triggerMarketEvent(eventDesc: string, currentStats: any) {
    try {
      ...
      // Generate orders and sync inventory/geo stats using only real Firestore data
      const newOrders = [];
      const currentInventory = [...(currentStats.inventory_items || [])];
      const currentGeoStats = { ...(currentStats.geo_stats || {}) };
      ...
      // Log ke Firebase
      await FirebaseLogger.logAgentAction('Manager', 'SYSTEM_EVENT', `Simulated: ${eventDesc}`);
