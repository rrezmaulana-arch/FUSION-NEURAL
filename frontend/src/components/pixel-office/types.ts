export interface ColorValue { h: number; s: number; b: number; c: number; colorize?: boolean; }
export {
  DEFAULT_COLS,
  DEFAULT_ROWS,
...
export interface Character {
  ...
  bubbleType: 'permission' | 'waiting' | 'speech' | null;
  /** Countdown timer for bubble (waiting: 2→0, permission: unused) */
  bubbleTimer: number;
  /** Text to show in speech bubble */
  bubbleText?: string;
  /** Pending reply text waiting for delay timer */
  pendingBubbleText?: string;
  /** Countdown until pending bubble is shown (seconds). When it reaches 0, bubble fires. */
  pendingBubbleDelay: number;
  ...
  outputTokens: number;
  /** Cooldown before this character can initiate another social interaction (seconds) */
  socialCooldown: number;
  /** Whether character is currently stationary doing work (for animations) */
  isWorking: boolean;
}
