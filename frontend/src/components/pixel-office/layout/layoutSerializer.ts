  ...
  for (const item of furniture) {
    ...
    if (entry.category === 'chairs') {
      if (entry.orientation === 'back') {
        ...
        // Use the bottom footprint row + character hit height so it sorts after the character
        zY = (item.row + entry.footprintH) * TILE_SIZE + 24;
      } else {
    ...
    // Items that sit on surfaces
    if (entry.canPlaceOnSurfaces) {
      // Render on top of the desk (assume desk is 2 tiles high = 32px), 
      // but behind any character standing in front of it.
      zY = item.row * TILE_SIZE + 32 + 1;
    }
