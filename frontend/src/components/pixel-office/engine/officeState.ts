import {
  ...
  SOCIAL_COOLDOWN_SEC,
  SOCIAL_INTERACT_CHANCE_PER_SEC,
  SOCIAL_INTERACT_RANGE_TILES,
...
export class OfficeState {
  ...
  setAgentActive(id: number, active: boolean): void {
    if (id === 2) active = true; // FORCE Manager always active!
    const ch = this.characters.get(id);
    if (ch) {
  ...
  /** Show a speech bubble with custom text above a character. Auto-dismisses after SPEECH_BUBBLE_DURATION_SEC. */
  showSpeechBubble(id: number, text: string): void {
    const ch = this.characters.get(id);
    if (!ch) return;
    ch.bubbleType = 'speech';
    ch.bubbleText = text;
    ch.bubbleTimer = SPEECH_BUBBLE_DURATION_SEC;
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      // Tick pending bubble delay — fires B's reply after A finishes speaking
      if (ch.pendingBubbleDelay > 0) {
        ch.pendingBubbleDelay -= dt;
        if (ch.pendingBubbleDelay <= 0 && ch.pendingBubbleText) {
          ch.bubbleType = 'speech';
          ch.bubbleText = ch.pendingBubbleText;
          ch.bubbleTimer = SPEECH_BUBBLE_DURATION_SEC;
          ch.pendingBubbleText = undefined;
          ch.pendingBubbleDelay = 0;
        }
      }

      // Tick social cooldown
      if (ch.socialCooldown > 0) {
        ch.socialCooldown = Math.max(0, ch.socialCooldown - dt);
      }

      // Mark isWorking when character is actively typing at their desk
      ch.isWorking = ch.state === 'type' && ch.isActive;
    }

    // Social interactions: wandering characters near each other may chat
  ...
  /** Check proximity between all wandering characters and trigger chat bubbles */
  private _updateSocialInteractions(dt: number): void {
    const chatA = [
      'Gimana projectnya? 🙂', 'Deploy kapan nih?', 'Kopi dulu bos~',
      'Ketemu bug lagi 😩', 'PR udah di-review?', 'Meeting jam 3 ya!',
      'Codenya clean banget!', 'Fix dulu ah...', 'Deadline besok 😅',
      'Ada snack di dapur!', 'Server lagi down?', 'Testing udah pass!',
      'Ngopi yuk! ☕', 'Dokumentasinya mana?', 'GG, shipped! 🚀',
    ];
    const chatB = [
      'Sip, gaspol! 💪', 'Bentar lagi kelar~', 'Makasih bro!',
      'Udah kok, tenang.', 'Siap! 15 menit lagi.', 'Oke noted! 👌',
      'Hehe iya nih 😂', 'Gas terus!', 'Lagi dicek nih...',
      'Mantap jiwa! 🔥', 'Iya udah stable.', 'Yeay! 🎉',
      'Yuk! Bareng?', 'Udah kelar tadi.', 'Finally! 🙌',
    ];

    // Characters must be wandering AND not already mid-conversation
    const chars = Array.from(this.characters.values()).filter(
      (c) =>
        !c.matrixEffect &&
        c.socialCooldown <= 0 &&
        c.bubbleType === null &&
        c.pendingBubbleDelay <= 0 &&
        c.state !== 'type', // don't interrupt typing
    );
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      for (let j = i + 1; j < chars.length; j++) {
        const b = chars[j];
        if (b.socialCooldown > 0 || b.bubbleType !== null || b.pendingBubbleDelay > 0) continue;

        const dist =
          Math.abs(a.tileCol - b.tileCol) + Math.abs(a.tileRow - b.tileRow);
        if (dist > SOCIAL_INTERACT_RANGE_TILES) continue;

        // Probability check per-frame (~1.5 chats/sec when close)
        if (Math.random() > SOCIAL_INTERACT_CHANCE_PER_SEC * dt) continue;

        // A speaks immediately
        const lineA = chatA[Math.floor(Math.random() * chatA.length)];
        const lineB = chatB[Math.floor(Math.random() * chatB.length)];

        a.bubbleType = 'speech';
        a.bubbleText = lineA;
        a.bubbleTimer = SPEECH_BUBBLE_DURATION_SEC;

        // B replies AFTER A finishes (~1.5s delay so they look like a conversation)
        b.pendingBubbleText = lineB;
        b.pendingBubbleDelay = SPEECH_BUBBLE_DURATION_SEC * 0.6; // fires midway

        // Apply cooldown so they don't loop instantly
        a.socialCooldown = SOCIAL_COOLDOWN_SEC + SPEECH_BUBBLE_DURATION_SEC;
        b.socialCooldown = SOCIAL_COOLDOWN_SEC + SPEECH_BUBBLE_DURATION_SEC;

        // Face each other during conversation
        if (a.tileCol < b.tileCol) { a.dir = 2; b.dir = 1; }       // A→right, B→left
        else if (a.tileCol > b.tileCol) { a.dir = 1; b.dir = 2; }  // A→left, B→right
        else if (a.tileRow < b.tileRow) { a.dir = 0; b.dir = 3; }  // A→down, B→up
        else { a.dir = 3; b.dir = 0; }                              // A→up, B→down

        // Stop both from walking while chatting
        if (a.state === 'walk') { a.path = []; a.state = 'idle'; a.wanderTimer = SOCIAL_COOLDOWN_SEC; }
        if (b.state === 'walk') { b.path = []; b.state = 'idle'; b.wanderTimer = SOCIAL_COOLDOWN_SEC; }

        break; // only one pair per character per frame
      }
    }
  }
