...
function renderBubbles(
  ...
  for (const ch of characters) {
    ...
    let alpha = 1.0;
    if ((ch.bubbleType === 'waiting' || ch.bubbleType === 'speech') && ch.bubbleTimer < BUBBLE_FADE_DURATION_SEC) {
      alpha = ch.bubbleTimer / BUBBLE_FADE_DURATION_SEC;
    }

    const sittingOff = ch.state === CharacterState.TYPE ? BUBBLE_SITTING_OFFSET_PX : 0;

    if (ch.bubbleType === 'speech' && ch.bubbleText) {
      ctx.save();
      ...
      const bubbleX = Math.round(offsetX + ch.x * zoom - bgWidth / 2);
      const bubbleY = Math.round(
        offsetY + (ch.y + sittingOff - BUBBLE_VERTICAL_OFFSET_PX) * zoom - bgHeight - 4
      );

      // Bubble background
      ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
      ctx.beginPath();
      ctx.roundRect(bubbleX, bubbleY, bgWidth, bgHeight, 6);
      ctx.fill();
      
      // Bubble tail
      ctx.beginPath();
      ctx.moveTo(bubbleX + bgWidth / 2 - 4, bubbleY + bgHeight);
      ctx.lineTo(bubbleX + bgWidth / 2, bubbleY + bgHeight + 5);
      ctx.lineTo(bubbleX + bgWidth / 2 + 4, bubbleY + bgHeight);
      ctx.fill();
      ...
      ctx.fillText(ch.bubbleText, bubbleX + bgWidth / 2, bubbleY + bgHeight / 2 + 1);
