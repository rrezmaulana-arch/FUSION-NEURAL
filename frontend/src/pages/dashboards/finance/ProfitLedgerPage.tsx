...
export default function ProfitLedgerPage() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'finance_transactions'), orderBy('created_at', 'desc'), limit(20));
    getDocs(q).then((snapshot) => {
      const data = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      if (data.length > 0) {
        const formatted = data.map((d: any) => ({ id: d.id, ...d, timestamp: d.created_at, amount: d.amount || 0, isPositive: d.transaction_type === 'INCOME' }));
        setOrders(formatted);
        setRecentOrders(formatted.slice(0, 8));

        // Mock financial report based on transactions
